...
function modulateNeuralSwarm(isRevert) {
    if (isRevert) {
        liveSeparation = 35;       
        liveActivity = 9.5;        
...
function appendCard(data) {
    ...
    modulateNeuralSwarm(data.isRevert);
    ...
    card.innerHTML = `
        ...
        <div class="card-content">
            <div class="meta-line">// NEURAL_NODE // OPERATOR: ${data.user} // COLLISION: ${data.isRevert ? 'REVERT_CRITICAL_ALERT' : 'STABLE'}</div>
...
function applyEntropy() {
    ...
    cards.forEach(card => {
        ...
        // 1. 【変更】画面内に見えている間（rect.top >= 0）は1文字も壊さず、完全に読める状態を維持
        // 画面の最上部を超えて上へ見切れていく（rect.top < 0）タイミングから崩壊を開始
        if (rect.top < 0) {
            
            // 2. 【変更】毎回確実に壊すのをやめ、30%の確率の時だけ進行（風化の速度を1/3に減速）
